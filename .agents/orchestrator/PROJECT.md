...
## Overview
cortex-enterprise is an enterprise-grade Criminal Network Analysis (CNA) system featuring scalable data ingestion, knowledge graph with entity resolution, high-performance graph analytics (influence, suspicion, proximity risk), automated insight & contradiction detection, and a modern web dashboard.
...
- Target Workspace: `c:\Users\NIRJHAR BARMA\Desktop\batcave\cortex-enterprise`
  ...
  - `demo-case-data/` or `data/`: Ingestion sample datasets and ground truth.
  - `tests/` or `backend/tests/`: Automated test suite (`test_ingestion_pipeline.py`, `test_graph_analytics.py`, `test_contradictions.py`, `test_ghost_nodes.py`, `test_e2e_integration.py`).
