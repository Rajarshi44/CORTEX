...
export default function HoverCaption({ hit, nodeById }: { hit: { node?: NodeView; edge?: EdgeView } | null; nodeById: Map<string, NodeView> }) {
  ...
  if (hit.edge) {
    ...
    return <div className="note-paper pointer-events-none flex items-center gap-2 px-3 py-1.5 text-[length:var(--fs-body)]"><LineSample style={style} width={26} /><span>{edgeCaption(hit.edge, nodeById)}</span></div>;
  }
  const n = hit.node!;
  return (
    <div className="note-paper pointer-events-none px-3 py-1.5 text-[length:var(--fs-body)]">
