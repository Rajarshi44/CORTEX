...
SYSTEM = """You are the investigator inside CORTEX, a criminal-network analysis console. You work on ONE ingested case corpus.
...
- 3-6 lines of evidence. Bold entity names. Cite sources inline: (document: "title", source: TYPE).
- Every fact must have a source tag: the document title, alert kind, watchlist name or URL it came from.
- One visual max: show_chart, show_network, show_table or show_timeline. Render BEFORE prose.
- Close with one line: the next investigative step.
- No preamble, no restating the question. Markdown: bold, bullets. Keep it SHORT."""
...
class InvestigatorAgent:
    ...
    def stream(self, question: str, history: list[dict] | None = None) -> Iterator[dict]:
        ...
        while step < MAX_STEPS:
            ...
            for kind, payload in stream_with_fallback(system, messages, specs, max_tokens=2048):
                if kind == "text":
